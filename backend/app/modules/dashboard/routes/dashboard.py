...
async def get_dashboard_summary(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    repos_stmt = select(Repository.sync_status).where(Repository.user_id == current_user.id)
    ...
    commit_stmt = select(CommitEvent).where(CommitEvent.user_id == current_user.id).order_by(desc(CommitEvent.created_at)).limit(5)
    ...
    # Fetch PR Drafts
    pr_stmt = select(PRDraft).where(PRDraft.user_id == current_user.id).order_by(desc(PRDraft.created_at)).limit(5)
    ...
    recent_pr_drafts = []
    pr_list_stmt = select(PRDraft.id, PRDraft.title, PRDraft.status).where(PRDraft.user_id == current_user.id).order_by(desc(PRDraft.created_at)).limit(5)
    pr_list_res = await db.execute(pr_list_stmt)
    for d_id, d_title, d_status in pr_list_res:
        recent_pr_drafts.append({
            "id": str(d_id),
            "title": d_title or "Untitled Draft",
            "status": d_status
        })
