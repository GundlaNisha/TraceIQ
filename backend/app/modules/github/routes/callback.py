...
async def github_app_callback(
    ...
    state: str = Query(
        ..., description="The user ID passed in the OAuth state parameter"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to authenticate with GitHub"
        )
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to fetch installation details"
            )
