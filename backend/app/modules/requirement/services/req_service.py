...
async def create_requirement(
    ...
    v1 = RequirementVersion(
        requirement_id=req.id, version_number=1, title=title, text=text
    )
...
async def update_requirement(
    db: AsyncSession, req: Requirement, title: str, text: str
) -> Requirement:
    ...
    new_version = RequirementVersion(
        requirement_id=req.id, version_number=req.version_number, title=title, text=text
    )
    from app.modules.audit.models.audit import AuditLog
