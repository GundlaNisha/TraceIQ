...
async def semantic_search(db: AsyncSession, query: str, repository_id: uuid.UUID, top_k: int = 10) -> list[dict]:
    ...
    stmt = (
        select(CodeChunk, RepositoryFile, CodeEmbedding.embedding.cosine_distance(query_vector).label("distance"))
    ...
    for chunk, repo_file, distance in result:
        ...
        items.append({
            ...
            "score": 1.0 - float(distance)  # Convert pgvector distance into a similarity score
        })
    return items
