...
async def _run_impact_analysis_async(job_id: str):
    async with AsyncSessionLocal() as db:
        try:
            ...
            job.progress = 100
            await db.commit()
            logger.info(f"Impact analysis {job_id} completed successfully.")

        except Exception:
            logger.exception(f"Impact analysis {job_id} failed")
            await db.rollback()
            # Mark the job as failed — do NOT re-raise; Celery would record a
            # second FAILURE state which is confusing and triggers any error
