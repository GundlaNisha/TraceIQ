        ...
        try:
            ...
            if draft.requirement_id:
                req_stmt = select(Requirement).where(
                    Requirement.id == draft.requirement_id
                )
            ...
            if draft.commit_event_id:
                diff_stmt = select(CommitDiff).where(
                    CommitDiff.commit_event_id == draft.commit_event_id
                )
                ...
                if diffs:
                    ...
                    for d in diffs:
                        summary_lines.append(
                            f"Modified: {d.file_path} (+{d.additions} -{d.deletions})"
                        )
                    diff_summary = "\n".join(summary_lines)
            ...
            findings_data = []
            if draft.commit_event_id:
                find_stmt = select(ReviewFinding).where(
                    ReviewFinding.commit_event_id == draft.commit_event_id
                )
            ...
            # 4. Dispatch to LLM
            ai_result = await dispatch_pr_generation(
                req_text, diff_summary, findings_data
            )
