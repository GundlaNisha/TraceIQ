...
async def run_evaluation(repo_id: str, github_repo_name: str, limit: int = 10):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for pr in merged_prs:
            ...
            actual_files = set([f.filename for f in pr.get_files()])
            
            # 1. Semantic search
            search_results = await semantic_search(session, requirement, repository.id, top_k=15)
            ...
            suggested_files = set([f.file_path for f in ai_result.impacted_files])
            ...
            precision = true_positives / len(suggested_files) if suggested_files else 0.0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Impact Engine using Historical PRs")
    parser.add_argument("--repo-id", type=str, required=True, help="UUID of the repository in the local DB")
    parser.add_argument("--github-repo", type=str, required=True, help="GitHub repo name, e.g. 'owner/repo'")
    parser.add_argument("--limit", type=int, default=10, help="Number of PRs to evaluate")
