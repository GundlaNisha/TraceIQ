...
def _derive_test_database_url() -> str:
    ...
    Defaults to an isolated test database (traceiq_test) to avoid wiping
    the active dev/production database during pytest runs.
    """
    explicit = os.getenv("TEST_DATABASE_URL")
    if explicit:
        return explicit
    return "postgresql+asyncpg://postgres:postgres@localhost:5432/traceiq_test"
